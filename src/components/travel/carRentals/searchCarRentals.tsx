...
import { CarRentalType } from '@/components/travel/carRentals/carRentalType'
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function SearchCarRentals() {
  const t = useTranslations('homepage.travel.search.carRentals')
  ...
  const handleSearch = async () => {
    ...
    try {
      const quotationData = {
        ...
        pickupDate: dateRange.from.toISOString().split('T')[0],
        returnDate: dateRange.to?.toISOString().split('T')[0],
        drivers: parseInt(drivers),
        carType: selectedType
      }

      const response = await fetch('/api/car-rental/quotes', {
        method: 'POST',
        headers: {
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <div>
                  <Select value={drivers} onValueChange={setDrivers}>
