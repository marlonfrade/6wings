    ...
    <ThreadPrimitive.Empty>
      <div className="flex w-full max-w-[var(--thread-max-width)] flex-grow flex-col">
        <div className="flex w-full flex-grow flex-col items-center justify-center">
          <Avatar>
            <AvatarImage src="/images/logos/6wings-logo.png" />
            <AvatarFallback>6W</AvatarFallback>
          </Avatar>
  ...
  return (
    <div className="mt-3 flex w-full items-stretch justify-center gap-4">
      <ThreadPrimitive.Suggestion
        ...
        prompt="Quais são as promoções do dia?"
        method="replace"
        autoSend
      >
        <span className="line-clamp-2 text-ellipsis text-sm font-semibold">
          {t('suggestions.promotions')}
        </span>
      </ThreadPrimitive.Suggestion>
      <ThreadPrimitive.Suggestion
        className="flex max-w-sm grow basis-0 flex-col items-center justify-center rounded-lg border p-3 transition-colors ease-in hover:bg-muted/80"
        prompt="Como faço para rastrear meu pedido?"
        method="replace"
        autoSend
      >
        <span className="line-clamp-2 text-ellipsis text-sm font-semibold">
          {t('suggestions.tracking')}
        </span>
      </ThreadPrimitive.Suggestion>
    </div>
  )
  ...
  return (
    <>
      <ThreadPrimitive.If running={false}>
        <ComposerPrimitive.Send asChild className="text-white">
          <TooltipIconButton
            tooltip={t('actions.send')}
            variant="default"
            className="my-2.5 size-8 p-2 text-white transition-opacity ease-in"
          >
